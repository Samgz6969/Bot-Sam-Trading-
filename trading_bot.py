...
class Config:
    ...
    poll_seconds: int = 3            # Cada cuanto revisa el mercado cada moneda (segundos)
    ...
    telegram_summary_every_n_cycles: int = 600  # resumen periodico por moneda (~cada 30 min con poll de 3s)
...
class SymbolTradingBot:
    ...
    def fetch_24h_change_pct(self) -> float:
        ...
        muestra la propia app/web de Binance), sin depender del timeframe elegido."""
    ...
    def step(self):
        ...
        # Arma las velas OHLC reales (ultimas N) para el grafico tipo Binance del panel
        candles_df = df.tail(self.cfg.price_history_maxlen)
        time_unix = candles_df["timestamp"].values.astype("datetime64[s]").astype("int64")
        candles = [
            {"time": int(t), "open": round(o, 6), "high": round(h, 6),
             "low": round(l, 6), "close": round(c, 6)}
            for t, o, h, l, c in zip(
                time_unix, candles_df["open"], candles_df["high"],
                candles_df["low"], candles_df["close"],
            )
        ]
        shared_state.push_candles(self.symbol, candles)

        try:
            pct_change_24h = self.fetch_24h_change_pct()
        except Exception as e:
            self.log.warning(f"[{self.symbol}] No se pudo obtener el %cambio 24h: {e}")
            pct_change_24h = None  # se mantiene el ultimo valor conocido, no rompe el ciclo
        ...
        if pct_change_24h is not None:
            update_fields["pct_change_24h"] = round(pct_change_24h, 2)

        shared_state.update(self.symbol, **update_fields)
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                self.log.error(f"[{self.symbol}] Error en el ciclo: {e}", exc_info=True)
