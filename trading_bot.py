...
class SymbolTradingBot:
    ...
    def _push_account_state(self, current_price: float, now=None):
        ...
        # PnL FLOTANTE: cuanto va ganando/perdiendo la posicion abierta AHORA MISMO,
        # en vivo, a medida que el precio sube o baja (0 si no hay posicion abierta).
        if self.account.in_position and self.account.entry_price:
            floating_pnl_usdt = round(self.account.position_qty * (current_price - self.account.entry_price), 2)
            floating_pnl_pct = round((current_price / self.account.entry_price - 1) * 100, 2)
        else:
            floating_pnl_usdt = 0.0
            floating_pnl_pct = 0.0
