...
try:
    ...
    # Si por cualquier motivo el paquete websocket-client no esta instalado
    # (por ejemplo, requirements.txt desactualizado en el repo), el bot NO debe
    # morir por eso -- sigue funcionando 100% con el respaldo REST, igual que
    # si el WebSocket estuviera bloqueado por Binance. Solo se pierde el
    # empuje de datos en tiempo real.
    websocket = None
...
class Config:
    ...
    watchdog_check_seconds: int = 5     # cada cuanto revisa el vigilante si hay datos recientes
    watchdog_stale_seconds: int = 12    # sin ticks WS: refresco RAPIDO de precio/% via REST
    watchdog_full_backfill_seconds: int = 60  # sin ticks WS: recarga completa de velas + senales
...
class BinanceWebSocketFeed:
    # data-stream.binance.vision es el equivalente en WebSocket de data-api.binance.vision:
    # el mismo espejo de solo-datos-publicos-de-mercado, pero sin la restriccion geografica
    # que SI tiene wss://stream.binance.com (confirmado en produccion: HTTP 451). Documentado
    # oficialmente por Binance como "Market Data Only URLs".
    WS_URL = "wss://data-stream.binance.vision/ws"
    RETRY_SECONDS_NORMAL = 5        # reintento normal (fallo de red transitorio)
    RETRY_SECONDS_BLOCKED = 1800    # reintento cuando Binance ya confirmo bloqueo geografico (30 min)

    def __init__(self, log, notifier: "TelegramNotifier" = None):
        self.log = log
        ...
        self._next_req_id = 1
        self._stop = False
        self._geo_blocked = False       # True si Binance ya devolvio un 451 (bloqueo por region)
        self._notified_blocked = False  # para avisar por Telegram solo UNA vez, no en cada intento
        self._down_since = None         # marca de tiempo desde que se cayo, para medir cuanto duro
    ...
    def _on_open(self, ws):
        # Si llegamos a abrir conexion, cualquier bloqueo previo ya no aplica
        # (por ejemplo si Render cambio de IP de salida).
        self._geo_blocked = False
        streams = self._desired_streams()
        if self._down_since is not None:
            downtime = (datetime.now(timezone.utc) - self._down_since).total_seconds()
            self.log.info(
                f"WebSocket de Binance RECONECTADO tras {downtime:.0f}s caido. "
                f"Suscribiendo a {len(streams)} streams..."
            )
            if self.notifier:
                self.notifier.send(
                    f"✅ El WebSocket en tiempo real de Binance se reconecto (estuvo caido "
                    f"~{downtime:.0f}s). Volviendo a datos instantaneos."
                )
    ...
    def _on_error(self, ws, error):
        # Binance responde con HTTP 451 cuando la IP de origen (la del servidor
        # donde corre el bot) esta geograficamente restringida -- el mismo motivo
        # por el que la API REST normal necesita el espejo data-api.binance.vision.
        # A diferencia de un fallo de red transitorio, este bloqueo es permanente
        # mientras el bot siga en ese mismo servidor/region, asi que no tiene
        # sentido reintentar cada 5 segundos para siempre.
        text = str(error)
        if "451" in text or "restricted location" in text:
            self._geo_blocked = True
            self.log.warning(
                "WebSocket de Binance: bloqueado por restriccion geografica (HTTP 451). "
                "El bot seguira funcionando normalmente via REST de respaldo."
            )
        else:
    ...
    def _on_close(self, ws, code, msg):
        if self._down_since is None:
            self._down_since = datetime.now(timezone.utc)
    ...
    def start(self):
        if not WEBSOCKET_AVAILABLE:
            self.log.warning(
                "El paquete 'websocket-client' no esta instalado (revisa requirements.txt "
                "en el repo). El bot continua funcionando normalmente solo con REST, sin "
                "el empuje de datos en tiempo real."
            )
            if self.notifier:
                self.notifier.send(
                    "ℹ️ Falta el paquete websocket-client en el servidor, asi que el "
                    "WebSocket en tiempo real no pudo iniciar. El bot sigue funcionando "
                    "con total normalidad via REST. Revisa que requirements.txt en tu "
                    "repo de GitHub incluya la linea 'websocket-client'."
                )
            return
    ...
    def _run_forever_loop(self):
        ...
        Si Binance ya confirmo un bloqueo geografico (451), espacia mucho mas
        los reintentos en vez de insistir cada 5 segundos sin parar."""
        while not self._stop:
            ...
            if self._geo_blocked:
                if not self._notified_blocked:
                    self._notified_blocked = True
                    self.log.warning(
                        "WebSocket de Binance bloqueado por region en este servidor. "
                        f"El bot sigue operando via REST de respaldo; se reintentara el "
                        f"WebSocket cada {self.RETRY_SECONDS_BLOCKED // 60} minutos."
                    )
                    if self.notifier:
                        self.notifier.send(
                            "ℹ️ El WebSocket en tiempo real de Binance esta bloqueado desde "
                            "este servidor (restriccion geografica, HTTP 451) -- igual que le "
                            "pasaba antes a la API REST normal. El bot sigue funcionando con "
                            "total normalidad usando el respaldo REST, solo que sin el empuje "
                            "instantaneo de precio. Se seguira intentando la conexion en vivo "
                            "cada 30 min por si cambia la situacion."
                        )
                time.sleep(self.RETRY_SECONDS_BLOCKED)
            else:
                self.log.warning(f"WebSocket de Binance desconectado. Reintentando en {self.RETRY_SECONDS_NORMAL}s...")
                time.sleep(self.RETRY_SECONDS_NORMAL)
...
class SymbolTradingBot:
    ...
    # WebSocket usa su equivalente data-stream.binance.vision por el mismo
    # motivo: wss://stream.binance.com SI dio bloqueo 451 en produccion
    # (confirmado en Render). Aun asi, el "watchdog" de abajo cae de vuelta
    # al REST si el WebSocket no trae datos por cualquier razon: el bot
    # nunca depende por completo del WebSocket para seguir operando.
    ...
    def on_kline_tick(self, k: dict):
        ...
        Binance manda esto ~1 vez por segundo mientras la vela se esta formando,
        no solo cuando cierra."""
        t = int(k["t"] // 1000)
        ...
        # Actualizacion LIGERA del grafico en cada tick (solo listas de Python, sin
        # pandas) para que la vela en formacion se vea viva en el panel sin costo real de CPU.
        # IMPORTANTE: esto NO cuenta como "tick fresco" para el vigilante -- solo mueve el
        # grafico, no el precio. Si solo esto siguiera llegando pero el stream de ticker
        # (que es el que trae el precio real) se hubiera cortado, el vigilante debe darse
        # cuenta igual y caer a REST -- por eso self.last_tick_at NO se toca aqui.
        shared_state.push_candles(self.symbol, candles_snapshot[-self.cfg.price_history_maxlen:])

        # El calculo de la senal de trading (SMA sobre un DataFrame de pandas) es mucho
        # mas costoso en CPU que lo anterior. Si se hiciera en CADA tick de vela en
        # formacion (~1 por segundo, x6 monedas) satura la CPU del servidor gratuito y
        # Render termina matando la instancia por "no responde" -- por eso SOLO se hace
        # cuando la vela realmente CIERRA. El precio en vivo y el take-profit/stop-loss
        # ya se manejan aparte, al instante, desde on_ticker_tick -- no se pierde
        # capacidad de reaccion con este cambio, solo se evita trabajo repetido inutil.
        if not is_closed or len(candles_snapshot) < self.cfg.long_window + 2:
            return

        self.last_tick_at = datetime.now(timezone.utc)  # esto SI es un precio fresco (cierre de vela)
        df = pd.DataFrame(candles_snapshot)
        df["timestamp"] = pd.to_datetime(df["time"], unit="s")
        self._process_tick(df, current_price=c, status="operando (WebSocket)")
    ...
    def on_ticker_tick(self, data: dict):
        ...
        try:
            ...
            return
        # Este SI es el tick que le importa al vigilante: es la fuente real del precio.
        # Si este stream especifico se corta (aunque el de velas siga vivo), queremos
        # que el vigilante lo detecte y caiga a REST -- por eso se marca aqui.
        self.last_tick_at = datetime.now(timezone.utc)
        ...
        self._push_account_state(price, status="operando (WebSocket)")
    ...
    def run(self):
        ...
        # IMPORTANTE: este hilo debe permanecer vivo. El trading real ocurre por
        # callbacks del WebSocket / watchdog; si run() termina, en el arranque
        # clasico el proceso principal hace join() y luego sale, matando todos
        # los daemon threads (Flask, Telegram, WebSocket) y provocando reinicios
        # en Render. Bloqueamos indefinidamente de forma barata.
        while True:
            time.sleep(3600)

    def _refresh_price_rest(self):
        """Actualizacion ligera de precio y %24h via REST (sin recargar todo el historial).
        Se usa cuando el WebSocket no esta empujando ticks, para que el panel y Telegram
        no se queden con un precio congelado respecto a Binance."""
        try:
            price = self.fetch_current_price()
        except Exception as e:
            self.log.warning(f"[{self.symbol}] REST precio falló: {e}")
            return
        shared_state.push_price(self.symbol, price)
        try:
            pct = self.fetch_24h_change_pct()
            ...
            self.log.warning(f"[{self.symbol}] REST %24h falló: {e}")
        try:
            self.check_risk_management(price)
        except Exception as e:
            self.log.warning(f"[{self.symbol}] Error TP/SL en refresh REST: {e}")
        self._push_account_state(price, status="operando (REST)")
        # No tocamos last_tick_at aqui: eso solo lo actualizan el WS o un backfill
        # completo, para que el vigilante sepa si el WebSocket sigue muerto.

    def _watchdog_loop(self):
        last_full_backfill_at = None
        rest_fallback_since = None  # marca desde cuando este simbolo esta en modo REST (para medir duracion y loguear la recuperacion)
        while True:
            time.sleep(self.cfg.watchdog_check_seconds)
            now = datetime.now(timezone.utc)
            age = None if self.last_tick_at is None else (now - self.last_tick_at).total_seconds()
            # Si el WebSocket esta vivo (ticks recientes), no hacemos nada.
            if age is not None and age <= self.cfg.watchdog_stale_seconds:
                if rest_fallback_since is not None:
                    downtime = (now - rest_fallback_since).total_seconds()
                    self.log.info(
                        f"[{self.symbol}] WebSocket se RECUPERO tras {downtime:.0f}s en modo REST. "
                        f"Volviendo a datos en tiempo real."
                    )
                    rest_fallback_since = None
                continue

            if rest_fallback_since is None:
                rest_fallback_since = now
                self.log.warning(
                    f"[{self.symbol}] WebSocket sin datos hace "
                    f"{'mas de ' + str(self.cfg.watchdog_stale_seconds) + 's' if age is None else f'{age:.0f}s'}. "
                    f"Cambiando a modo REST de respaldo."
                )

            # 1) Refresco RAPIDO de precio/% para que el panel coincida con Binance
            try:
                self._refresh_price_rest()
            except Exception as e:
                self.log.warning(f"[{self.symbol}] Fallo el refresco REST de precio: {e}")

            # 2) De vez en cuando, backfill completo (velas + senales)
            need_full = (
                last_full_backfill_at is None
                or (now - last_full_backfill_at).total_seconds() >= self.cfg.watchdog_full_backfill_seconds
            )
            if need_full:
                self.log.info(f"[{self.symbol}] Recargando historial completo via REST (backfill periodico)...")
                try:
                    self.backfill()
                    last_full_backfill_at = datetime.now(timezone.utc)
                except Exception as e:
                    self.log.warning(f"[{self.symbol}] Fallo el respaldo REST completo del vigilante: {e}")
...
def start_services(start_flask_thread: bool = False):
    """Inicializa bots, WebSocket, Telegram y (opcionalmente) el hilo de Flask.
    ...
    """
    global ws_feed, _services_started
    ...
    ws_feed = BinanceWebSocketFeed(root_log, notifier=shared_notifier)
    ws_feed.start()
    root_log.info("Feed de WebSocket en tiempo real iniciado (data-stream.binance.vision).")
