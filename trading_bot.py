...
class SymbolTradingBot:
    ...
    def on_ticker_tick(self, data: dict):
        ...
        shared_state.update(self.symbol, status="operando (WebSocket)")
        try:
    ...
    def _watchdog_loop(self):
        ...
        while True:
            ...
            try:
                ...
                shared_state.update(self.symbol, status="operando (REST)")
            except Exception as e:
            ...
            if need_full:
                self.log.warning(
                    f"[{self.symbol}] Sin datos del WebSocket en "
                    f"{'todo el tiempo' if age is None else f'{int(age)}s'}. "
                    f"Recargando historial via REST..."
                )
                try:
                    self.backfill()
...
function updateCard(symbol, d) {
  ...
  if (updEl) updEl.textContent = d.last_update ? ('precio: ' + d.last_update) : '--';
